...
function processContains(item, list, callback) {
  return callback(list.some(arrVal => item === arrVal));
...
function getFullNames(runners) {
  let name = [];
  runners.forEach(ahh => name.push(`${ahh.last_name}, ${ahh.first_name}`));
  return name
}
...
function firstNamesAllCaps(runners) {
  let cap = runners.map(arr => arr.first_name.toUpperCase().toString());
  return cap
}
...
function getRunnersByTShirtSize(runners, tShirtSize) {
  const ans = runners.filter(runner => runner.shirt_size === tShirtSize)
  return ans
...
function tallyUpDonations(runners) {
  return runners.reduce((acc, cv) => acc + cv.donation, 0);
}
